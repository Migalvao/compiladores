...
program: functionsAndDeclarations                                               {printf("programa\n");$$ = myprogram = insert_element("Program", $1);}
        ;

functionsAndDeclarations: functionDefinition                                    {printf("funcsNDec\n");$$ = $1;}
...
functionDefinition: typeSpec functionDeclarator functionBody                    {printf("funcDef\n");$2-> next -> next = $3; $1-> next = $2; $$ = insert_element("FuncDefinition", $1);}
                    ;
...
typeSpec: CHAR                                                                  {$$ = insert_element("Char", NULL); sprintf(type_string,"Char" );}
        | INT                                                                   {printf("int\n");$$ = insert_element("Int", NULL); sprintf(type_string,"Int" );}
...
functionDeclarator: id_token LPAR parameterList RPAR                                  {printf("declarator\n");$$ = $1; $$ -> next = $3;}
                    ;
...
parameterDeclaration: typeSpec                                                  {printf("param\n");$$ = insert_element("ParamDeclaration", $1);}
                    | typeSpec id_token                                         {$1 -> next = $2; $$ = insert_element("ParamDeclaration", $1);}
                    ;

functionBody: LBRACE RBRACE                                                     {$$ = insert_element("FuncBody", NULL);}
            | LBRACE declarationsAndStatements RBRACE                           {printf("funcBody\n");$$ = insert_element("FuncBody", $2);}
            ;

declarationsAndStatements: statement                                            {printf("statement\n");$$ = $1; aux=$1->children; printf("%s\n", $1->type); while(aux){ printf("%s - ", aux->type); aux=aux->next; }         }
                        |  declaration                                          {$$ = $1;}
                        |  declarationsAndStatements statement                  {if(! $1 -> next) {$1 -> next = $2;} else {aux =$1 -> next; while(aux->next)aux=aux->next; aux-> next = $2;  } $$ = $1;}
                        |  declarationsAndStatements declaration                {printf("+declaration\n");if(! $1 -> next) {$1 -> next = $2;} else {aux =$1 -> next; while(aux->next)aux=aux->next; aux-> next = $2;  } $$ = $1;}
                        ;
